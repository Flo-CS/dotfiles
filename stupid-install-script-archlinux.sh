...
if ! grep -q "AUTOGENERATED Default profile for user" ~/.profile; then
	echo "# AUTOGENERATED Default profile for user" >>~/.profile
...
if [[ $set_hyprland == "y" ]]; then
	if ! grep -q "# AUTOGENERATED Hyprland autostart" ~/.profile; then
		echo "# AUTOGENERATED Hyprland autostart" >>~/.profile
		echo "if uwsm check may-start; then" >>~/.profile
		echo "	exec uwsm start hyprland.desktop" >>~/.profile
		echo "fi" >>~/.profile
	fi
fi
